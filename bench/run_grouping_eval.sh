#!/usr/bin/env bash
# ============================================================
# NovGauge end-to-end grouping evaluation
#
# Workflow:
#   Step 1  run_grouping_bench  — Predict paper groups with an LLM
#   Step 2  evaluate_grouping   — Four-stage evaluation
#             Stage 1: Grouping accuracy (pairwise F1 per dimension)
#             Stage 2: Hallucination rate (evidence checks for predicted groups)
#             Stage 3: Mismatch rate (reasoning checks for non-hallucinated groups)
#             Stage 4: Verified F1 (both evidence and reasoning pass verification)
#
# Usage:
#   bash bench/run_grouping_eval.sh             # Full workflow (Step 1 + Step 2)
#   bash bench/run_grouping_eval.sh --eval-only # Skip Step 1 and run Step 2
#                                               # (use after run_grouping_bench has completed)
# ============================================================
...
EVAL_N_SAMPLES="${EVAL_N_SAMPLES:-1}"             # Number of samples (report mean +/- std when >1)
...
EVAL_MAX_FULL_TOKENS="${EVAL_MAX_FULL_TOKENS:-12000}"

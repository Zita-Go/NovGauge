"""
Check whether evidence_a / evidence_b returned by the LLM are genuine verbatim
substrings of the paper content that was supplied in the prompt.

Outputs three rates:
  verbatim_rate   – strict substring match (after Unicode / ligature normalisation)
  artifact_rate   – matches after additionally stripping common PDF extraction
                    artifacts (broken words, footnote numbers, image tags)
  hallucination_rate – pairs that fail BOTH checks; these are likely fabricated

Usage:
  python bench/evaluation/check_evidence.py \\
    --results results/negatives_crossgroup/openai_gpt-5.4/full.json \\
    --inputs  data/negatives.json \\
    --mode    full

  # Multiple result + input file pairs
  python bench/evaluation/check_evidence.py \\
    --results results/positives_all/openai_gpt-5.4/full.json \\
             results/negatives_crossgroup/openai_gpt-5.4/full.json \\
    --inputs  data/positives.json \\
             data/negatives.json \\
    --mode    full

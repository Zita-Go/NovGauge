"""
Run pairwise inference on positive and negative examples using three dimension-specific prompts.

Available content modes (multiple modes can be evaluated in one run):
  abstract        Use only the abstract supplied in the metadata
  abstract+intro  Use the abstract and introduction extracted from Markdown
  full            Use full Markdown, excluding the references section and subsequent content

Example usage:
  python bench/evaluation/run_bench.py \\
    --input data/positives.json \\
    --models openai/gpt-5.4 google/gemini-3.1-pro \\
    --content-modes abstract abstract+intro full \\
    --dims method problem \\
    --output-dir results/positives_test
"""
...
def get_content(paper: dict, mode: str, max_tokens: int) -> tuple[str, str]:
    """Return (content_type_label, content_text).

    content_type_label fills the <CONTENT_TYPE_A/B> prompt placeholders.
    max_tokens: token limit for the content of this paper.
    Prefer the configured content cache; extract content at runtime on a cache miss.
    """
...
def _thinking_extra_body(model: str, thinking_budget: int) -> dict | None:
    """Return extra_body for the configured maximum reasoning level of each model.

    thinking_budget acts as a switch: 0 disables reasoning; positive values enable the maximum configured level.
    Configured maximum reasoning levels:
      OpenAI GPT-5.5/5.4    → reasoning_effort: xhigh
      OpenAI o3/o4-mini     → reasoning_effort: high (no xhigh for the o-series)
      DeepSeek V4           → reasoning_effort: max (explicitly disabled when the budget is 0)
      Gemini 3.1-Pro/Flash  → thinkingLevel: high
      Kimi / GLM            → thinking.type: enabled (on/off only)
      MiniMax               → reasoning_split: true (reasoning is enabled by default)
      Qwen API              → enable_thinking: true (omit the budget to use the model maximum)
      Claude                → thinking.type: adaptive (model-adaptive reasoning)
      Qwen3 local vLLM       → chat_template_kwargs.enable_thinking: true
    """
    m = model.lower()
...
def call_llm(client: OpenAI, model: str, prompt: str,
             ...
             max_retries: int = 3,
             thinking_budget: int = 0) -> tuple[dict | None, str | None, str | None]:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--thinking-budget",     type=int, default=0,
                   help="Reasoning budget_tokens (0 disables reasoning)")
    return p.parse_args()

"""
Check whether the evidence strings returned by run_grouping_bench are genuine
verbatim substrings of the paper content supplied in the prompt.

For each predicted group in a grouping result, every paper's evidence is
classified as:
  verbatim       – strict substring match (after Unicode / ligature normalisation)
  artifact       – matches after additionally stripping common PDF extraction
                   artifacts (broken words, footnote numbers, image tags)
  hallucination  – fails BOTH checks; likely fabricated

A group is then classified as:
  "verbatim"      – ALL papers' evidence is verbatim
  "artifact"      – at least one paper is artifact, none is hallucination
  "hallucination" – at least one paper's evidence fails both checks

Usage:
  python bench/evaluation/check_evidence_grouping.py \\

"""
Four-stage evaluation pipeline for the NoveltyBench grouping benchmark.

Stage 1 — Grouping Accuracy (pairwise F1 per dim)
  For each (item_id, dim) where eval_valid=True, compute pairwise precision /
  recall / F1 by converting predicted and ground-truth groups to pairwise sets.
  Reports: dim, n_items, avg_P, avg_R, avg_F1.

Stage 2 — Hallucination Rate (predicted groups only)
  For each predicted group, check whether each paper's evidence is a verbatim
  substring of the paper content used in the prompt.
  Reports: dim, n_groups, verbatim%, artifact%, hallucination%.
  Also splits by correct_group (all pairs in GT) vs incorrect_group (has FP pairs).

Stage 3 — Mismatch Rate (non-hallucinated predicted groups)
  For each group not flagged as hallucination, ask the LLM judge whether the
  stated reason is logically supported by the evidence.
  Reports: dim, n_checked, supported%, mismatch%.
  Saves reasoning check results to output_dir/reasoning_check_grouping.json.

Stage 4 — Verified F1
  Credit true-positive predictions only when their evidence and reasoning pass verification.
...
Usage:
  python bench/evaluation/evaluate_grouping.py \\
    ...
    --output-dir  results/grouping_openai_gpt-5.4_full/eval_pipeline
"""
...
def stage4_verified_f1(
    ...
    """Stage 4 — Verified F1 for grouping.

    A predicted group's pairs count as 'verified' only if:
      1. The group is NOT hallucinated  (Stage 2 passed)
      2. The group's reasoning is supported=True  (Stage 3 passed)

    Verified F1 penalises hallucinated / mismatch / unchecked groups by treating
    their intra-group pairs as unverified TPs (they still cost precision/recall).

    Reports: Verified P/R/F1 per dim, TP breakdown (verified/halluc/mismatch/no_check).
    """
    import statistics as _stats

"""
Four-stage evaluation pipeline for NoveltyBench.

Stage 1 — Similarity judgment accuracy
  For each (pair, dim), compare pred vs label.
  Metrics: Precision / Recall / F1 / Accuracy per dimension.

Stage 2 — Hallucination rate  (pred=1 records only)
  Check whether evidence_a / evidence_b are verbatim substrings of paper content.
  Reports hallucination rate separately for:
    TP: pred=1, label=1  (correct predictions)
    FP: pred=1, label=0  (wrong predictions)

Stage 3 — Mismatch rate  (pred=1 AND evidence not hallucinated)
  LLM judge checks whether the stated Reason is logically supported by the evidence.
  Mismatch rate = supported=False / total checked.
  Interpretation: supported=False → reliable failure signal;
                  supported=True  → necessary but not sufficient.
...
Usage:
  python bench/evaluation/evaluate.py \\
    --pos-results results/new_prompt_full/positives/openai_gpt-5.4/full.json \\
    --neg-results results/new_prompt_full/negatives_crossgroup_fixed/openai_gpt-5.4/full.json \\
    --pos-inputs  data/positives.json \\
    --neg-inputs  data/negatives.json \\
    --mode        full \\
    --base-url    $NOVGAUGE_BASE_URL \\
    --api-key     <key> \\
    --model       openai/gpt-5.2 \\
    --output-dir  results/eval_pipeline
"""
...
def stage4_verified_f1(
    ...
    A TP counts as 'verified' only if:
      1. Evidence is NOT hallucinated  (Stage 2 passed)
      2. Reasoning is supported=True   (Stage 3 passed)

    Reports Verified Precision / Recall / F1 per dim, and a TP breakdown:
      TP_verified | TP_halluc | TP_mismatch | TP_no_check
    """
    import statistics as _stats
